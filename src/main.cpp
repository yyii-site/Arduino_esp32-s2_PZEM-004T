...
// To manage ESP_WMParameter objects
ESP_WMParameter *AIO_SERVER_DATA_FIELD;
ESP_WMParameter *AIO_SERVERPORT_DATA_FIELD;
ESP_WMParameter *AIO_USERNAME_DATA_FIELD;
ESP_WMParameter *AIO_KEY_DATA_FIELD;

ESP_WMParameter* DATA_FIELD[ ] =
{
...
void TEMPERtoMQTT()
{
  ...
  {
    Serial.println(F("Temperature publish ok"));        // T means publishing OK
  }
  else 
  {
    Serial.println(F("Temperature publish failed"));        // F means publishing failure
  }
}
...
void PZEMtoMQTT()
{
  ...
  JsonObject PZEMdata = jsonBuffer.to<JsonObject>();
  PZEMdata["temperature"]   = ds18b20_celsius;
  PZEMdata["voltage"]   = pzem.voltage();
  PZEMdata["current"]   = pzem.current();
  PZEMdata["power"]     = pzem.power();
  PZEMdata["energy"]    = pzem.energy();
  PZEMdata["frequency"] = pzem.frequency();
  PZEMdata["pf"]        = pzem.pf();
  String StrBuff;
  serializeJson(jsonBuffer, StrBuff);
  Serial.println(StrBuff);
  if(Pub_PowerMeter->publish(StrBuff.c_str()))
  {
    Serial.println(F("Value to PowerMeter feed sucessfully sent!"));
  }
  else 
  {
    Serial.println(F("Failed to send value to PowerMeter feed!"));
  }
}
...
void check_status()
{
  ...
  static ulong checkPZEM_timeout    = 0;
...
#define PZEMCHECK_INTERVAL    120000L
  ...
  if ((current_millis > LEDstatus_timeout) || (LEDstatus_timeout == 0))
  {
    ...
    if(digitalRead(LED_PIN) == LED_ON)
      LEDstatus_timeout = current_millis + LED_INTERVAL/100;
    else
  ...
  if ((current_millis > checkPZEM_timeout) || (checkPZEM_timeout == 0))
  {
    float voltage = pzem.voltage();
    if(isnan(voltage)){
        Serial.println("Error reading PZEM voltage");
    }
    else
    {
      Serial.print("PZEM Voltage: ");    Serial.print(voltage);
      Serial.println("V");
    }
    checkPZEM_timeout = current_millis + PZEMCHECK_INTERVAL;
  }
}
...
void createNewInstances()
{
  ...
  if(!Pub_PowerMeter)
  {
    Serial.print(F("Creating new MQTT_Pub_Topic,  Pub_PowerMeter = "));
    Serial.println(MQTT_Pub_Topic);
    
    Pub_PowerMeter = new Adafruit_MQTT_Publish(mqtt, MQTT_Pub_Topic_Power.c_str());
 
    Serial.print(F("Creating new Pub_PowerMeter object : "));
...
void wifi_manager() 
{
  ...
  digitalWrite(LED_PIN, LED_ON); // turn the LED on by making the voltage LOW to tell us we are in configuration mode.
  ...
  digitalWrite(LED_PIN, LED_OFF); // Turn LED off as we are not in configuration mode.
...
void ds18_loop(void) {
  ...
  ds18b20_celsius = celsius;
}

void taskOneWire(void *parameter)
{
  while (1)
  {
    ds18_loop();
...
void setup()
{
  ...
  xTaskCreate(taskOneWire, "taskOneWire", 2000, NULL, 10, NULL);
  digitalWrite(LED_PIN, LED_OFF); // Turn led off as we are not in configuration mode.   
}
